...
NEXT TASK: T2.58 — mTLS for Kafka (FR-1.10): a client certificate and key,
           which the shared tlsconf already builds for every driver, so the
           work is what Kafka adds — a SASL_SSL or SSL listener beside the
           plaintext ones, and whether a certificate alone can authenticate
           (Kafka's SSL principal) with no SASL mechanism at all. Then the
