...
NEXT TASK: T2.89 — export and import a connection set as JSON, with
           secrets excluded by default (FR-1.13). T2.88 just built the
           import half of a neighbouring problem and its rule applies
           here too: what leaves must not carry a password, and what
           is already saved must not be doubled by reading the same
           file twice (ADR-0111).
           T2.88 stays [~]: DBGate and TablePlus are not done and its
           line says exactly what each would need. DBGate needs the
           owner to install it so the file it writes can be observed,
           because its storage is undocumented and its source may not
           be read here.
           Still true, because no task names it: there is no way in
           from the window for an SSH tunnel, a cloud identity or an
           import. Three features now have a finished driver half and
           no user interface, and that gap outlives every task that
           made it.
...
LAST DONE: 2026-09-22 — connections read out of the tools somebody already uses —
