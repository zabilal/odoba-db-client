...
NEXT TASK: T2.73 — per-topic key/value decoder selection, remembered
           (FR-13.7). Read the task against what T2.68 already built and
           proved: the choice is kept by connection, topic and field, only a
           choice somebody made is remembered, and one that no longer fits
           falls back. If nothing is left to write, say so with a pointer
...
LAST DONE: 2026-09-19 — JSON Schema read by stripping the five bytes that
