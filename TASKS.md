...
NEXT TASK: T2.71 — Protobuf decoding (FR-13.7). Avro reads by its schema
           now (T2.70) and Protobuf still refuses in its own words, with a
           live test holding that boundary. A descriptor is not a schema
           text: what a registry holds for Protobuf has to be compiled
           before anything can be read by it.
...
LAST DONE: 2026-09-19 — a record read by its schema: Avro decoded through a
