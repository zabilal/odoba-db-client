...
NEXT TASK: the rest of T2.8 (a table with no key says why, and Choose
           a Key…, FR-4.7), then T2.9. What is
           open in Phase 1 waits on other work: T1.58 on the MongoDB and
           Redis drivers, T1.14's scroll position on Fyne.
[~] tasks: T1.14 and T2.8 partly done; each line says what is open.
...
LAST DONE: 2026-09-11 — SQLite tables known by their key or rowid,
           and a statement must change exactly one row (T2.8 in part,
           ADR-0034). Before it: changes reverted, and asked about before
...
- [~] **T2.8** Row-identity detection; refuse edit without a key, offer to nominate one → FR-4.7 — *SQLite knows a table by its primary key, or else its rowid, which the browse then selects; a statement changing more than one row fails as one changing none does (ADR-0034). Open: saying why a table cannot be edited, and Choose a Key…*
