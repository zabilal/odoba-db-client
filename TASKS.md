...
NEXT TASK: T2.88 — import connections from DBeaver, DBGate, TablePlus
           and DataGrip (FR-1.12), already marked [~]: read its line
           for what is open before starting. Reading another tool's
           own configuration file is not reading its source, so the
           clean-room rule does not bar the DBGate half; what it bars
           is taking the answer from DBGate's code rather than from a
           file it wrote.
           Still true after T2.87, because no task names it: the
           connection editor has no SSH section and no cloud section,
           so a tunnel and a cloud identity can each be configured
           only by editing the settings file by hand. Both driver
           halves are done — FR-1.9 and FR-1.14 — and the way in is
           not. That gap now outlives two tasks.
...
LAST DONE: 2026-09-22 — a token minted from an identity nobody typed — an AWS profile or
