...
NEXT TASK: T2.79 — a topic's configuration (FR-13.12): showing what it is
           set to, with what somebody set apart from what the broker
           defaults to. Altering it is written and claimed already (T2.78):
           AlterTopicConfig, incremental so that changing one setting leaves
           the others where they were. What is missing is reading it.
           model.Topic has a Config field of model.ConfigEntry that nothing
           fills, and ConfigEntry.IsOverride already draws the distinction
           the requirement asks for; kadm's DescribeTopicConfigs returns
           each setting with the source it came from. The topic's structure
           arm is where it goes — and ConfigEntry.Sensitive says a value
           comes back redacted, which it must stay.
...
LAST DONE: 2026-09-22 — topics made, unmade and reshaped, each asked about
