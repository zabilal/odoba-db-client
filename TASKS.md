...
NEXT TASK: T2.66 — the message detail view (FR-13.8): one record opened on
           its own, with its value decoded, its headers as a table, the raw
           bytes in hex, and copy. What a key or a value means is still a
           decoder's business (T2.68); this is the frame around it.
...
LAST DONE: 2026-09-19 — a log followed: a read that waits rather than ending,
