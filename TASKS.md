...
NEXT TASK: T2.50 — the CQL editor: highlighting comes from sqllex's CQL
           dialect, which the driver now names, and completion from the
           schema cache the SQL editors already use (T2.27-T2.29). Then
           T2.51 (partition-key-aware paging, which is what a table needs
           before it is browsable), T2.52 (the consistency selector) and
           T2.53 (conformance green). A Queryer to run a statement is what
           Capabilities.Query waits on, and what T2.50 will want.
...
LAST DONE: 2026-09-18 — the CQL dialect: how a name is written, where a
