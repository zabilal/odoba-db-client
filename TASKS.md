...
NEXT TASK: T2.74 — the Schema Registry browser (FR-13.14): subjects, their
           versions, the schema text, compatibility mode, and a diff between
           two versions. The client reads all of these already (T2.69); what
           is missing is somewhere to see them. KindSubject is claimed by
           nothing yet, so the tree is where this starts.
...
LAST DONE: 2026-09-19 — a remembered reading made to outlast the first
