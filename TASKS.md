...
NEXT TASK: T2.78 — topic administration (FR-13.12): creating a topic,
           deleting one, adding partitions, and altering configuration. All
           of it is AccessDDL rather than AccessWrite. Settle one thing
           before writing any of it: source.StreamAdmin still bundles these
           with ResetOffsets (T2.80), and conformance ties Stream.TopicAdmin
           and Stream.ResetOffsets alike to that one interface, so claiming
           the first as it stands means implementing a reset nobody has asked
           for. ADR-0107 split the reading half off for exactly this reason
           and the same cut may be wanted again — decide that first. Adding
           partitions changes key-to-partition mapping for every record
           written afterwards, which the interface must say before it
           happens, not after.
...
LAST DONE: 2026-09-22 — a record written to a topic: the first thing this
