...
NEXT TASK: T2.57 — AWS MSK IAM (FR-1.11), over franz-go's sasl/aws: a
           mechanism that signs with AWS credentials rather than holding a
           secret of its own, so the question is where those credentials
           come from (a profile, the environment, a role) and how much of
           that this application should ask for. Then T2.58 (mTLS) and the
           ...
           keytab, franz-go shipping no Kerberos mechanism at all.
           Two Kafka containers: ikigai-kafka (59092, plaintext) and
           ikigai-kafka-sasl (9092 plaintext, 9094 SASL: PLAIN, SCRAM-256
           and -512, OAUTHBEARER), both required by gate.sh. Handing
           franz-go a logger is still unclaimed work.
...
LAST DONE: 2026-09-18 — a Cassandra table paged: where its pages ended
