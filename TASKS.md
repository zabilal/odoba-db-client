...
NEXT TASK: T2.80 — resetting a consumer group's offsets (FR-13.13): to the
           earliest, the latest, a timestamp, or a named offset. It is
           AccessAdmin rather than AccessDDL, and source.OffsetResetter and
           source.ResetRequest already declare it — the interface was cut
           for this in T2.78, so claiming Stream.ResetOffsets implements one
           interface and promises nothing beyond it. It is the most
           destructive thing here: it changes what a running consumer reads
           next, which is why FR-13.13 puts it behind the production
           guardrail without exception, and why the interface has to say
           what will happen before it happens rather than after. kadm's
           CommitOffsets is the call. Read source.ResetRequest before
           designing the form: its shape decides what the form has to ask.
...
LAST DONE: 2026-09-22 — a topic's settings read and shown, with what
