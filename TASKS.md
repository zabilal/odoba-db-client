...
NEXT TASK: T2.67 — filtering records client-side (FR-13.9): by key, by
           header, or by a predicate over a decoded JSON value. A broker
           filters nothing, so this is applied to what has already been
           read, and the driver already refuses a filter in those words.
...
LAST DONE: 2026-09-19 — one record read whole: the forms its bytes admit and
