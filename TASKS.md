...
NEXT TASK: T2.72 — JSON Schema decoding (FR-13.7). The last of the three
           languages a registry holds. A JSON record is already readable as
           JSON without any schema, so what this adds is the schema's own
           account of it — and what it must not do is make a record that
           reads perfectly well today read worse.
...
LAST DONE: 2026-09-19 — Protobuf read by a schema compiled in process, the
