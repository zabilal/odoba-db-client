...
LAST DONE: 2026-09-11 — an import's rows written into its table, a batch
           a transaction, as a task (T2.21's inserting, ADR-0049). Before
           it: a dry run over a whole file to import, listing each value
...
- [ ] **T2.21** Modes: insert / upsert / replace / append-to-new-table → FR-10.6 — *inserting done: Import in the panel writes the rows through each source's Writer, 500 rows a transaction, as a task that says how long is left after a dry run; new rows need no key (`transfer.Load`, ADR-0049). Replace, upsert and a new table to come*
