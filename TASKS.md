...
NEXT TASK: T2.53 — conformance green for Cassandra: the capability suite
           run against a real cluster, which is what is left of 2.G. The
           driver claims a relational paradigm, a query language, server
           filtering and sorting, and no count; what it does not claim —
           writing, transactions, exact counts — the suite must skip rather
           than fail.
