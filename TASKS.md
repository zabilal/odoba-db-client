...
NEXT TASK: T2.68 — decoders for what a record carries (FR-13.7). Raw bytes,
           hex, UTF-8 text and JSON are already the forms a record is read in
           (T2.66); this adds Avro, Protobuf and JSON Schema through a
           Confluent Schema Registry, and remembers the choice per topic.
...
LAST DONE: 2026-09-19 — a topic's records filtered where a broker will not
