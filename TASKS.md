...
NEXT TASK: the grid: Copy As CSV/JSON/Markdown/INSERT (T1.52), the cell viewer
           (T1.53), column resize and hide (T1.48), type-aware rendering (T1.50);
           then Phase 1's other [~].
...
LAST DONE: 2026-09-11 — cell selection, and copying cells as tab-separated text
           (T1.51, T1.52 in part, ADR-0016).
```
...
- [~] **T1.52** Copy as TSV/CSV/JSON/INSERT/Markdown → FR-3.7 — *TSV done: ⌘C and Edit › Copy Cells, exact values, up to 100,000 rows; CSV, JSON, Markdown and INSERT are open*
