...
NEXT TASK: T2.51 — partition-key-aware paging: a page of a Cassandra table
           is where the last one ended, which the cluster hands back as a
           paging state, and until Browse can walk one no node is browsable.
           Then T2.52 (the consistency selector) and T2.53 (conformance
           green).
...
LAST DONE: 2026-09-18 — CQL run: a console's own session, a script guarded
