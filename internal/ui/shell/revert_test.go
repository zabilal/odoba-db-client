...
func TestRevertCellsPutsBackWhatWasRead(t *testing.T) {
	...
	if _, ok := tb.pending.Value(r1, 1); ok {
	...
	if tb.pending.Len() != 1 || !strings.HasSuffix(tb.footer.Text, " · 1 pending change") {
		t.Errorf("a row with no change left is unchanged: %d, %q", tb.pending.Len(), tb.footer.Text)
	}
...
func TestRevertRowsUndoesDeletionsAndTakesOutNewRows(t *testing.T) {
	...
	if tb.pending.State(r1) != model.RowUnchanged || tb.pending.State(r2) != model.RowUnchanged {
		t.Error("rows read are as they were read, a deletion undone")
	}
	if added := tb.pending.Added(); len(added) != 1 || added[0][1] != "kept" || tb.model.Added() != 1 {
		t.Errorf("the new rows selected go, the other stays: %v", added)
	}
	if tb.pending.Len() != 1 {
		t.Errorf("%d changes left", tb.pending.Len())
	}
}
...
func TestDiscardAllAsksThenForgetsEveryChange(t *testing.T) {
	...
	if tb.pending.Len() != 0 || tb.model.Added() != 0 || tb.review.Visible() || fx.s.canReview() {
		t.Errorf("every change forgotten: %d, %d new rows", tb.pending.Len(), tb.model.Added())
	}
...
func TestClosingATabWithChangesAsks(t *testing.T) {
	...
	tapOnTop(t, fx, "Cancel")
	if len(fx.s.open) != 1 || tb.pending.Len() != 1 {
