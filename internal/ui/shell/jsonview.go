...
func (s *Shell) newJSON(t *tab, g *grid.TableGrid, holder *fyne.Container) *jsonView {
	...
	j.prev.Importance, j.next.Importance, back.Importance = widget.LowImportance, widget.LowImportance, widget.LowImportance
	head := container.NewBorder(nil, nil, container.NewHBox(j.prev, j.next), back, j.where)
...
// valueJSON writes one value, indented to sit inside a document.
