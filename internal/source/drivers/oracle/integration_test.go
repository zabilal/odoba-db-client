...
var fixture = []string{
	...
	// The conformance suite's writable table, which wants an id the
	// server numbers from 1 when it is not given — an identity, here.
	//
	// A table of its own, and its columns quoted so that they are
	...
	// column no ordinary Oracle table has. The writes table above keeps
	// the names Oracle would have given it, so that the driver's own
	// tests are about the spelling its users will have.
	`CREATE TABLE conform (
	   "id" NUMBER(10) GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY,
	   "name" VARCHAR2(40) DEFAULT 'none' NOT NULL,
	   "n" NUMBER(10))`,
...
func TestConformance(t *testing.T) {
	...
	conformance.Run(t, conformance.Target{
		...
		Browsable: people,
		Writable:  model.NewRef(model.KindTable, "IKIGAI", "CONFORM"),
...
func TestLiveTheExplorerWalksTheService(t *testing.T) {
	...
	if tables.Badge == nil || tables.Badge.Text != "9" {
		t.Errorf("the tables folder counts %+v, want the nine in the fixture", tables.Badge)
	}
