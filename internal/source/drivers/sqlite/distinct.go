...
func (s *sqliteSource) Distinct(ctx context.Context, ref model.ObjectRef, column string, filters []source.Filter, limit int) ([]source.DistinctValue, error) {
	stmt, err := s.buildDistinct(ref, column, filters, limit)
	if err != nil {
		return nil, err
	}
