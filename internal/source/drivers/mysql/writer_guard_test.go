...
func TestEveryWritePathIsGuarded(t *testing.T) {
	...
	ro := &mysqlSource{cfg: source.ConnectionConfig{Guard: source.Guard{ReadOnly: true}}}
	prod := &mysqlSource{cfg: source.ConnectionConfig{Guard: source.Guard{Environment: source.EnvProduction}}}

	guardcheck.Check(t, ro, prod, map[string]func(*mysqlSource, bool) error{
		"Apply": func(s *mysqlSource, c bool) error {
		...
		"LoadRows": func(s *mysqlSource, c bool) error {
